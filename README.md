...
To ensure that the refinement process preserves the intended behaviour of the system, we perform equivalence checking between the original AADL model given in our paper and its refined AADL model produced by the RAMSES tool. It is supposed that the RAMSES generated model should have the same observational behavior with its original one. This is confimed by checking their corresponding pi-calculus code in MWB. We present bellow the refined AADL Model dervied by Ramses, its pi-calculus code generated by our AADLtoPiCalculus tool, and the results of verification in MWB.
